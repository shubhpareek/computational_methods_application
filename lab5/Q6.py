...
def nthchebyshevpolynomial(n):
    ...
    if(n == 0):
        return tn_1
    tn = Polynomial([0, 1])
    tn_plus_1 = Polynomial([1])

    # t_n+1 = 2x.t_n - t_n-1 using recursion
    for i in range(1, n):
        tn_plus_1 = Polynomial([0, 2])*tn - tn_1
...
def initiall5_cheb():
    n = 5
    # 5 polynomials
    poly_list = [nthchebyshevpolynomial(i) for i in range(n)]
    matrix = np.zeros((n, n))
    for i in range(n):
        for j in range(i, n):
            def function(x):
                return (1/math.sqrt(1-(x**2))) * (poly_list[i][x]*poly_list[j][x])
            # integrating for range -1 to 1
            val, _ = integrate.quad(function, -1, 1)
            # rounding values 
            val = np.round(val, 2)
            #to demonstrate orthogonality i store results in matrix
            matrix[i][j] = val
            matrix[j][i] = val
    # printing matrix
    print(matrix)
