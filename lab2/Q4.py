...
# of vertices with a particular degree.
    ...
    #will return size of most largest component in size and the second largest component in size
    def oneTwoComponentSizes(self):
        ...
        #will be used to implement queue data structure
        queue = []
        ...
        #iterate on all  vertices then calculate their connected component ,
        for nd in self.setofvertex:

            #this makes possible that each connected component is visited once only.
            if vis[nd] == 0:
                queue.append(nd)
                vis[nd] = True

            #size of connected component that we will visit now .
            conn_size = 0
            ...
            while queue:
                ...
                #iterate on adjacency linked list.
                while(temp is not None):
            ...
            #if this components size is greater than largest component that means, this itself is the largest component.
            if conn_size >= ccfirst:
                ccsecon = ccfirst
                ccfirst = conn_size
            #if this components size is not greater than largest but, is greater than second largest then it is the second largest component.
            elif conn_size >= ccsecon:
                ccsecon = conn_size

        return [ccfirst, ccsecon]
...
class ERRandomGraph(UndirectedGraph):
    ...
    """this will be used to verify if “If p < 0.001, the Erdős-Rényi random graph G(1000, p) will almost surely have only
    small connected components. On the other hand, if p > 0.001, almost surely, there will be a single giant component containing a positive fraction of the vertices.”"""
    
    def checkalmostsure(self):
        probvalues = np.linspace(0.0, 0.008, num=100)

        #for plotting probabillity
        xPoints = []
        #for largest component size 
        y1Points = []
        #for second largest component size
        y2Points = []

        #iterate on probablillities.
        for pr in probvalues:
            #for taking average of largest size in this probabillity
            lsavg = 0
            #for taking average of second largest size in this probabillity
            slsavg = 0
            #50 iterations are done for each probabillity.
            for i in range(0, 50):
                erdograp = ERRandomGraph(len(self.setofvertex))
                erdograp.sample(pr)
                (L, SL) = g.oneTwoComponentSizes()
                slsavg += (SL/len(self.setofvertex))
                lsavg += (L/len(self.setofvertex))

            #till now these variables just had the sum, now we convert them to average .
            lsavg /= 50
            slsavg /= 50

            #also keep updating the graph points .
            y1Points.append(lsavg)
            y2Points.append(slsavg)
            xPoints.append(pr)

        #for plotting the graph .
        plt.plot(xPoints, y2Points, color="purple")
        plt.plot(xPoints, y1Points, color="green")
        plt.grid()
        ...
        plt.axvline(x=math.log(len(self.vertices))/len(self.vertices),
                    color="yellow", label="Theoretical threshold")
        plt.xlabel("p")
        plt.ylabel("fraction of nodes")
