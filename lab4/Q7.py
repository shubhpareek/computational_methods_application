...
if __name__ == "__main__":
    """
    getting coefficients of taylor series of sin(x)*e^x
    and creating the polynomial from these coefficients.
    """
    # Creating the polynomial
    fx = Polynomial([0, 1, 1, 1 / 3, 0, -1 / 30, -1 / 90, -1 / 630, 0, 1 / 22680, 1 / 113400])
    taylorarea = fx.area(0, 1 / 2)
    #integration of sin(x)*e^x is exp(x) * (sin(x) - cos(x)) / 2
    realarea = np.exp((1/2)) * (np.sin((1/2)) - np.cos((1/2))) / 2 - np.exp((0)) * (np.sin((0)) - np.cos((0))) / 2
    error = abs(realarea - taylorarea)

    # Printing the results
    print("The computed area under the curve is:",taylorarea)
    print("The actual area under the curve is:", realarea)
